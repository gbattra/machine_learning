...
predictionErrors = zeros(length(C_set), length(sigma_set));

for c = 1:length(C_set)
  for s = 1:length(sigma_set)
    model = svmTrain(X, y, C_set(c), @(x1, x2) gaussianKernel(x1, x2, sigma_set(s)));
    predictions = svmPredict(model, Xval);
    predictionErrors(c,s) = mean(double(predictions ~= yval));
  end
end

[c,s]=find(predictionErrors==min(min(predictionErrors)));
